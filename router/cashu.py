...
async def refund_balance(amount_msats: int, key: ApiKey, session: AsyncSession) -> int:
    ...
    # Attempt to send the refund first
    result = await WALLET.send_to_lnurl(
        key.refund_address,
        amount=amount_sats,
    )

    # After a successful send, atomically deduct the balance to avoid race
    # conditions when multiple refunds are triggered concurrently.
    stmt = (
        update(ApiKey)
        .where(col(ApiKey.hashed_key) == key.hashed_key)
    ...
    exec_result = await session.exec(stmt)  # type: ignore[call-overload]
    await session.commit()
