...
async def pay_out() -> None:
    """Calculates the pay-out amount based on spent balance and donation rate."""
    try:
        ...
        async with create_session() as session:
            balance = (
                await session.exec(select(func.sum(col(ApiKey.balance))).where(ApiKey.balance > 0))
            ).one()
            ...
            logger.info("Payout sent: owners %s sats, devs %s sats", owners_draw, devs_donation)
...
async def credit_balance(cashu_token: str, key: ApiKey, session: AsyncSession) -> int:
    ...
    logger.info("Credited %s msats to key %s", amount_msats, key.hashed_key[:10])
    return amount_msats


async def check_for_refunds() -> None:
    """Periodically check for API keys that are eligible for refunds."""
