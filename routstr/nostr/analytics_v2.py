...
def _outcome_vector(outcome: LedgerOutcome) -> MetricVector:
    sources = tuple(
        "estimated"
        if getattr(outcome, f"{name}_source") == "estimated"
        else "reported"
        if getattr(outcome, f"{name}_source") == "reported"
        or getattr(outcome, f"{name}_observed") is True
        else "missing"
...
def _validate_outcome(outcome: LedgerOutcome) -> None:
    ...
    for name in (
        ...
        if value is not None and not isinstance(value, bool):
            raise AnalyticsV2Error(f"{name} must be a boolean or null")
    for name in ("input", "output", "cache_read", "cache_creation"):
        source = getattr(outcome, f"{name}_source")
        if source not in (None, "reported", "estimated", "missing"):
