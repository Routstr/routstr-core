        ...
        async with self._session_factory() as session:
            ...
            sources = tuple(
                case(
                    ...
                    (or_(source == "reported", observed.is_(True)), "reported"),
                    else_="missing",
                )
                for source, observed in (
                    (
                        col(TerminalOutcome.input_source),
                        col(TerminalOutcome.input_observed),
                    ),
                    (
                        col(TerminalOutcome.output_source),
                        col(TerminalOutcome.output_observed),
                    ),
                    (
                        col(TerminalOutcome.cache_read_source),
                        col(TerminalOutcome.cache_read_observed),
                    ),
                    (
                        col(TerminalOutcome.cache_creation_source),
                        col(TerminalOutcome.cache_creation_observed),
                    ),
                )
            )
            measured = and_(
            ...
            rows = tuple(
                LedgerOutcome(
                    ...
                    input_observed=input_source == "reported",
                    output_observed=output_source == "reported",
                    cache_read_observed=cache_read_source == "reported",
                    cache_creation_observed=cache_creation_source == "reported",
