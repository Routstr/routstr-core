...
from .terminal_outcomes import SessionFactory
...
async def get_ledger_usage_dashboard(
    ...
    *,
    ...
    for dimension in _DIMENSIONS:
        stored_source = col(getattr(TerminalOutcome, dimension + "_source"))
        source = case(
            (stored_source == "estimated", "estimated"),
            (stored_source == "reported", "reported"),
            (
                (stored_source.is_(None) | (stored_source == "missing"))
                & col(getattr(TerminalOutcome, dimension + "_observed")).is_(True),
                "reported",
            ),
            else_="missing",
        )
