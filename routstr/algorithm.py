...
def calculate_model_cost_score(model: "Model") -> float:
    ...
    image_cost = getattr(pricing, "image", 0.0) * 0.1  # Weight lower as not every request uses images
...
def should_prefer_model(
    ...
    if should_replace:
        candidate_provider_name = getattr(candidate_provider, "upstream_name", "unknown")
...
def create_model_mappings(
    ...
    def _maybe_set_alias(alias: str, model: "Model", provider: "UpstreamProvider") -> None:
        ...
        if not existing_model:
            ...
            if should_prefer_model(model, provider, existing_model, existing_provider, alias):
                model_instances[alias] = model
                provider_map[alias] = provider
