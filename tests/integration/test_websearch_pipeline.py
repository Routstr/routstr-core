...
from routstr.websearch.types import SearchResult, WebPage, WebSearchContext
...
def setup_mocks(mock_model, mock_key, mock_search_result):
    ...
    with (
        ...
        mock_wm.enhance_request_with_web_context.side_effect = (
            simulate_successful_web_search
        )
...
@pytest.mark.asyncio
async def test_websearch_non_streaming_auth(
    setup_mocks, model_id, mock_upstream_response_json
):
    ...
    with patch(
        "httpx.AsyncClient.send", new_callable=AsyncMock, return_value=mock_response
    ) as mock_send:
        response = await proxy(
            request=mock_request, path="v1/chat/completions", session=AsyncMock()
        )
        ...
        assert (
            "The Chiefs won the Super Bowl." in forwarded_body["messages"][0]["content"]
        )
...
async def test_websearch_streaming_xcashu(setup_mocks, model_id):
    ...
    mock_response.aread = AsyncMock(
        return_value=b"".join([c async for c in mock_aiter_bytes()])
    )
    ...
    with (
        patch(
            ...
            new_callable=AsyncMock,
            return_value=(5000, "msat", "http://mint"),
        ),
        patch(
...
@pytest.mark.asyncio
async def test_websearch_non_streaming_xcashu(
    setup_mocks, model_id, mock_upstream_response_json
):
    ...
    mock_response.aread = AsyncMock(
        return_value=json.dumps(mock_upstream_response_json).encode()
    )
