...
from routstr.core.terminal_outcomes import SessionFactory
...
def _row(
    ...
    *,
    ...
    return TerminalOutcome(
        ...
        input_observed=source == "reported",
        output_observed=source == "reported",
...
async def test_measured_average_uses_paired_reported_requests_including_free_and_zero(
    ...
    input_only.input_source, input_only.input_observed = "reported", True
    output_only = _row(
        "output-only",
        at,
        input_tokens=0,
    ...
    output_only.output_source, output_only.output_observed = "reported", True
    async with sessions() as session:
        session.add_all(
            [
...
async def test_measured_average_distinguishes_unknown_from_measured_zero(
    ...
    if scenario == "unpaired":
        ...
        input_only.output_source, input_only.output_observed = "missing", False
        output_only = _row(
            "output", NOW - timedelta(hours=1), cache_read=0, cache_write=0
        )
        output_only.input_source, output_only.input_observed = "missing", False
...
async def test_measured_average_normalizes_sources_without_admitting_estimates(
    sessions: SessionFactory,
    source: str,
    observed: bool | None,
) -> None:
    ...
    row.input_observed = row.output_observed = observed
    async with sessions() as session:
        session.add(row)
        await session.commit()
    result = await get_ledger_usage_dashboard(
    ...
    eligible = source == "reported" or (source == "missing" and observed is True)
