...
async def test_plain_text_error_is_normalized(
    ...
    # An upstream 5xx is reported as 424 so the caller does not read it as this
    # node's health (CORE-UPSTREAM-5XX-NOT-NODE-DOWN); the provider's own
    # status stays discoverable.
...
# --------------------------------------------------------------------------- #
# Acceptance: CORE-UPSTREAM-5XX-NOT-NODE-DOWN
#
# An upstream-attributable failure must answer 424 + error.code
# UPSTREAM_UNAVAILABLE + header X-Routstr-Error-Scope: upstream, and keep the
# provider's own status in error.upstream_status. A genuine node fault must
# keep answering 500 with no scope header. Rate limits keep 429.
# --------------------------------------------------------------------------- #
...
async def test_upstream_5xx_is_attributed_to_the_upstream(
    ...
    payload: dict[str, Any] = json.loads(bytes(response.body))
    assert payload["error"]["code"] == UPSTREAM_UNAVAILABLE
    # The provider's own status survives so operators can still tell what the
