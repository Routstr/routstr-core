...
def test_generic_upstream_error_reports_424() -> None:
    ...
    The 502 was indistinguishable from a node-side gateway failure, so callers
    marked this node down for an upstream blip (CORE-UPSTREAM-5XX-NOT-NODE-DOWN).
    """
    from routstr.core.exceptions import UpstreamError
    from routstr.payment.helpers import create_upstream_error_response
    ...
    # The provider's own status is preserved for operators.
...
async def test_5xx_wrapped_rate_limit_is_classified(
    ...
    payload: dict[str, Any] = json.loads(bytes(response.body))
    assert payload["error"]["code"] == UPSTREAM_RATE_LIMIT
    # Still upstream-attributable, so the scope header is present…
    assert response.headers[ERROR_SCOPE_HEADER] == ERROR_SCOPE_UPSTREAM
    # …and the raw 5xx the provider actually sent stays discoverable.
