...
@pytest.mark.parametrize(
    ...
    # An upstream 5xx is re-reported as 424 (CORE-UPSTREAM-5XX-NOT-NODE-DOWN);
    # a 429 keeps its retry hint; 200 passes through untouched.
...
async def test_cashu_pin_reaches_http_transport(
    ...
    if client_status == 424:
        # Upstream-attributed: the caller can see it is not this node.
...
async def test_pinned_exception_does_not_fall_back() -> None:
    ...
    # Pinned: no fallback. The failure belongs to the pinned upstream, so the
    # caller sees 424 + the scope header rather than a 503 that reads as this
    # node being down.
...
#
# Upstream-attributable failures must be reported as 424 + UPSTREAM_UNAVAILABLE
# with the X-Routstr-Error-Scope: upstream header, must stay retryable across
# candidates, and must never make a node fault look like an upstream one.
# --------------------------------------------------------------------------- #
