...
def test_measured_cohort_cannot_be_inferred_from_separate_source_counts() -> None:
    measured = _outcome(
        output_observed=True, output_source="reported", output_tokens=20
    )
    missing = _outcome(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"input_source": "missing", "input_observed": False}, (0, 0)),
        ({"output_source": "missing", "output_observed": False}, (0, 0)),
...
@pytest.mark.parametrize("source", [None, "reported", "estimated", "missing"])
@pytest.mark.parametrize("observed", [None, False, True])
def test_provenance_normalization_preserves_estimates_and_observed_only_rows(
    source: str | None, observed: bool | None
) -> None:
    changes: dict[str, Any] = {
        f"{name}_{field}": value
        for name in ("input", "output", "cache_read", "cache_creation")
        for field, value in (("source", source), ("observed", observed))
    }
    row = (
        _aggregate(
            [
